 ...
 * This example will make a repeated reading at the read rate on channel 0 and auto-set CAPDAC for each channel
 **************************************************************
 * Setup
...
void setup() {
  ...
  fdc.resetDevice();
  delay(500);
  setCAPDAC(measA, pin1, pin1, measCont, &capdacA);
}

void loop() {  
  delay(500); // This can be removed, the read rate delay is set within relativeCapacitance()
  float capA = absoluteCapacitance(measA, pin1, pin1, measCont, &capdacA);
  Serial.println(capA);
}
