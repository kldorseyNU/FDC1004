 ...
 * This example will make a differential re-triggered reading for capacitors between chan0 and chan1 
 * CAPDAC is not set for differential measurements, so the maximum differential capacitance is +/- 15 pF
 **************************************************************
 * Setup
 ...
 * Place a 10 pF from chan0 to GND
 * You should read a value of between 8 to 12 pF
 **************************************************************
...
void setup() {
  ...
  fdc.resetDevice();
  delay(500);
  setCAPDAC(measA, pin1, pin2, measSingl, &capdacA);
}

void loop() {  
  delay(500); // This can be removed, the read rate delay is set within relativeCapacitance()
  float capA = absoluteCapacitance(measA, pin1, pin2, measSingl, &capdacA);
  Serial.println(capA);
}
